...
def predict_testimages(data_dir,sess,predicted_labels,images_ph,imagesize=IMAGE_SIZE):
		...
		pred_labels =[]
		test_images, test_labels = load_datasets(data_dir,IMAGE_SIZE)
...
def cnn_model(input_x,input_y,keep_prob):
		weight1=tf.Variable(tf.truncated_normal(shape=[3,3,1,64],stddev=5e-2))
		kernel1=tf.nn.conv2d(input_x,weight1,[1,1,1,1],padding='SAME')
		bias1=tf.Variable(tf.constant(0.0,shape=[64]))
		conv1=tf.nn.relu(tf.nn.bias_add(kernel1,bias1))
		pool1=tf.nn.max_pool(conv1,ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')
		
		weight2=tf.Variable(tf.truncated_normal(shape=[3,3,64,128],stddev=5e-2))
		...
		weight3=tf.Variable(tf.truncated_normal(shape=[16*16*128,1024],stddev=0.04))
...
def predict_testrandomimages(data_dir,sess,predicted_labels,images_ph,imagesize=IMAGE_SIZE):
		# Load the test dataset.
		images, labels = load_datasets(data_dir,IMAGE_SIZE)
...
def main(_):
		...
		images, labels = load_datasets_norm(train_data_dir,IMAGE_SIZE)
